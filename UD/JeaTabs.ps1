    ...
    New-UDTabs -Tabs {
        foreach ($endpoint in $session:endpoints) {
            New-UDTab -Text $endpoint.Name -Content {
                ...
                # LOCAL DEV MODE
                $session:tasks = Get-Command Set-Content -CommandType Cmdlet | Where-Object { $_.Parameters } | Select-Object -First 550 -Property Name, Parameters, CommandType
                ...
                New-UDDynamic -Id dyn1 -Content {
                    ...
                    New-UdTable -Data $tasks -Columns $columns -Id tbl -Sort -Filter -Search
                } #-AutoRefresh
            } -Stacked -Id dyn1
        }
    } -RenderOnActive        
